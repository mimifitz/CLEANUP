...
router.get("/services", (req, res) => {
    ...
    db("SELECT * FROM services ORDER BY id ASC;")
        .then((results) => {
...
router.get("/services/:id", (req, res) => {
    ...
    db(`SELECT * FROM services WHERE id=${req.params.id};`)
        .then((results) => {
            res.send(results.data);
        })
        .catch((err) => res.status(500).send(err));
...
router.put("/services/:id", (req, res) => {
    ...
    db(`UPDATE services set type =
            '${req.body.type}', name ='${req.body.name}', description ='${req.body.description}', availability ='${req.body.availability}', location = '${req.body.location}', userId =${req.body.userId} WHERE id = ${req.params.id};`)
        .then((results) => {
            if (results.error) {
                res.status("ERROR! TRY AGAIN").send({
